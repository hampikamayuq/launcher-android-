...
| 10 — Sobre o papel de parede | v1.1.0 | Entregue como planejado na seção 9. Diferenças: os favoritos em lista entram como um item único da lista (para o arraste de reordenar ter matemática local); a estrela do índice é a `Icons.Outlined.Star` do `material-icons-core` (sem o contorno vazado, que exigiria o pacote estendido). Gestos validados só por compilação e prévias, não em aparelho. |
